...
//import { computeHashOnElements , hash.getSelectorFromName } from '../hash';
import { hash, merkle, number, shortString } from "starknet";
//import { BigNumberish, number.isHex, toBN, toHex } from '../number';
...
function getHex(value: number.BigNumberish): string {
  try {
    return number.toHex(number.toBN(value));
  } catch (e) {
    if (typeof value === 'string') {
      return number.toHex(number.toBN(shortString.encodeShortString(value)));
    }
...
export function prepareSelector(selector: string): string {
  return number.isHex(selector) ? selector : hash.getSelectorFromName(selector);
}
...
export const getTypeHash = (types: TypedData['types'], type: string): string => {
  return hash.getSelectorFromName(encodeType(types, type));
};
...
export const encodeValue = (
  ...
  if (
    ...
    return [type, hash.computeHashOnElements(structHashes)];
  }
  ...
  if (type === 'merkletree') {
    ...
    const { root } = new merkle.MerkleTree(structHashes as string[]);
    return ['felt', root];
  }

  if (type === 'felt*') {
    return ['felt*', hash.computeHashOnElements(data as string[])];
  }
...
export const getStructHash = <T extends TypedData>(
  ...
  return hash.computeHashOnElements(encodeData(types, type, data)[1]);
};
 ...
 * @return {string}
 */
export const getMessageHash = (typedData: TypedData, account: number.BigNumberish): string => {
