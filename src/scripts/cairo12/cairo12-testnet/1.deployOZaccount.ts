// create a new Cairo 2.1.0 OZ account 0.7.0 in testnet
...
async function main() {
    ...
    //process.exit(2);
    console.log("CallData.compile object=",CallData.compile({
            recipient: OZcontractAddress,
            amount: cairo.uint256(5 * 10 ** 15)
        }));
    console.log("CallData.compile array=",CallData.compile([[
            OZcontractAddress,
            5 * 10 ** 15,0
        ]]));
    // const res = await account0.execute(
    // {
    //     contractAddress: addrETH,
    //     entrypoint: 'transfer',
    //     calldata: [
    //       OZcontractAddress,
    //       cairo.uint256(5 * 10 ** 15),
    //     ],
    //   });
      const res = await account0.execute(
        {
